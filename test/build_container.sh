...
if [ "$RESTY_IMAGE_BASE" == "ubuntu" ] || [ "$RESTY_IMAGE_BASE" == "debian" ]; then
  cp output/*${RESTY_IMAGE_TAG}.${BUILDPLATFORM}.deb docker-kong/ubuntu/kong.deb
  BUILD_DIR="ubuntu"
elif [ "$RESTY_IMAGE_BASE" == "alpine" ]; then
  cp output/*.${BUILDPLATFORM}.apk.tar.gz docker-kong/alpine/kong.tar.gz
  BUILD_DIR="alpine"
elif [ "$RESTY_IMAGE_BASE" == "centos" ] || [ "$RESTY_IMAGE_BASE" == "amazonlinux" ]; then
  cp output/*.${BUILDPLATFORM}.rpm docker-kong/centos/kong.rpm
  BUILD_DIR="centos"
fi
...
if [ "$RESTY_IMAGE_BASE" == "rhel" ]; then
  ...
  cp output/*.rhel${RESTY_IMAGE_TAG}.${BUILDPLATFORM}.rpm docker-kong/rhel/kong.rpm
  BUILD_DIR="rhel"
else
  sed -i 's/^FROM .*/FROM '${DOCKER_NAMESPACE}'\/'${ARCHITECTURE}-${RESTY_IMAGE_BASE}:${RESTY_IMAGE_TAG}'/' docker-kong/${BUILD_DIR}/Dockerfile
fi
